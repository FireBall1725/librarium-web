            ...
            <div className="flex items-start justify-between gap-3 px-4 py-3">
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <button onClick={() => startEdit(loc)}
                  className="p-1.5 rounded text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-surface-inset transition-colors">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                  </svg>
                </button>
                <button onClick={() => handleDelete(loc.id)}
                  className="p-1.5 rounded text-gray-400 hover:text-danger hover:bg-surface-inset transition-colors">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {result && (
              <div className="border-t border-line-subtle px-4 py-3 space-y-3 text-xs">
                ...
                {result.linked?.length > 0 && (
                  <div>
                    <p className="font-medium text-content-secondary mb-1">Linked</p>
                    <ul className="space-y-0.5">
                      {result.linked.map((l, i) => (
                        <li key={i} className="text-content-tertiary flex gap-2">
                ...
                {result.unlinked?.length > 0 && (
                  <div>
                    <p className="font-medium text-content-secondary mb-1">Unmatched files</p>
                    <ul className="space-y-0.5">
                      {result.unlinked.map((f, i) => (
                        <li key={i} className="font-mono text-content-muted truncate">{f.file_path}</li>
                      ))}
                    </ul>
                  </div>
                )}
