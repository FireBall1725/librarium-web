...
export default function EditBookModal({ libraryId, book, onClose, onSaved, initialTab = 'details' }: Props) {
  ...
  const [seriesRows, setSeriesRows] = useState<SeriesRowDraft[]>(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Series, same diff.
      await saveSeriesRows(callApi, libraryId, bookId, seriesRows, initialSeries.current)
      ...
      <div className="w-full max-w-xl rounded-xl bg-surface shadow-xl flex flex-col">
        ...
        {tab === 'details' && (
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <SeriesRowsField libraryId={libraryId} rows={seriesRows} onChange={setSeriesRows}
